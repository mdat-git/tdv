...
from inspections_lakehouse.etl.etl_002_scope_events_intake.pipeline import run_pipeline, PIPELINE


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETL 002 - Scope events intake (vendor-grain)")
    ...
    p.add_argument(
        "--event-type",
        required=True,
        choices=["removal", "move_to_helo"],
        help="Which event file is this? 'removal' or 'move_to_helo'",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    try:
        run_pipeline(run, input_path=args.input, vendor=vendor, event_type=event_type)
